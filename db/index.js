        ...
        .prompt([
            {
                name: "title",
                type: "input",
                message: "What id the title of the new role?",
            },
            {
                name: "salary",
                type: "input",
            ...
            },
            {
                ...
                type: "input",
                message:
                    "What is the Department ID for this new role? Slect 1 for Sales, 2 for Enginering, 3 for Finance, 4 for Leagl.",
                choices: [1, 2, 3, 4],
            },
        ])
        .then(function (answer) {
            ...
            connection.query(
                ...
                function (err, res) {
                    if (err) throw err;
                    console.log(`Sucessfully Added Role: ${answer.title}`);
                    runSearch();
                }
            )
        }
    ...
    inquirer
        .prompt([
            ...
            },
            {
                name: "newRoleDeptID",
                type: "list",
                message: "What department will the belong to? Sletct 1 for Sales, 2 for Engineering, 3 for Finance, 4 for Legal.",
                choices: [1, 2, 3, 4]
            },
        ])
        .then(function (answer) {
            var query = "UPDATE role SET title = ?, salary= ?, department_id = ? WHERE ID = ?";
            connection.query(query, [answer.newRoleTitel, answer.newRoleSalary, answer.newRoleDeptID, parseInt(answer.currentEmployeeID)], function (err, res) {
                if (err) throw err;
